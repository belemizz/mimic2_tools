function [  ] = save_data_of( id_list, metric_list, save_graph, duration, exclude_unreliable)
      ...
      for idx = 1:split_num
        ...
        for didx= 1:length(metric_list);
          if ~isempty(info(didx).LengthTime)
            ...
            subplot(length(metric_list) * n_pid_per_page, 1, length(metric_list) * (pidx-1) + didx);
            hold on;
            data{didx} = sig_of_int
            time{didx} = tm_from_base
        ...
        split = strsplit(nurl, '/')
        data_path = sprintf('%s/%s-%d.mat', data_folder, split{length(split)}, idx)
        save(data_path, 'data', 'time')
      end
    end
  end
end
