...
def main():
    ...
    def transform_values(input_values):
        ...
        temp = []
        for item in input_values[0]:
            if len(item[0])>0 and len(item[1])>0:
                temp.append([float(item[0][0]), float(item[1][0])])
        return numpy.array(temp)

    positive_x = transform_values(expire_values)
    negative_x = transform_values(recover_values)
#    positive_y = numpy.ones(positive_x.shape[0])
    ...
    print shared_y.eval()
    show_logistic_regression.show_logistic_regression(shared_x, shared_y, 0.002, 1000000, 500000)
